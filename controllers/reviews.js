...
const getReviewsForProduct = function (productId) {
  return new Promise((resolve, reject) => {
    const queryString = `
    SELECT prod.name AS product_name, rev.id, rev.rating, rev.recommended, rev.subject, rev.is_helpful, rev.is_not_helpful, exp.play_experience, exp.difficulty, exp.value, exp.build_time, users.name, users.age FROM products AS prod
